(function (root) {
  ...
  const api = { pointsForPercentage, percentageForMark, normalizeHouse, detectStructure, buildScoreboard, normalizeHeader, resetScoreboardState, round2, buildConsolidatedWorkbook };
  ...
  async function downloadExcel(){
    ...
    const slug=[state.details.className,state.details.section,state.details.exam,"Scoreboard"].join("_").replace(/[^a-z0-9]+/gi,"_").replace(/^_|_$/g,""); await XLSX.writeFile(wb,`${slug}.xlsx`);
  }
  function printableReport(selectedHouse) {
    ...
    return `<!doctype html><html><head><meta charset="utf-8"><title>AcadPulse Scoreboard</title><style>@page{size:landscape;margin:10mm}body{font:10px Arial,sans-serif;color:#142b25}header{text-align:center;margin-bottom:16px}h1{margin:0;font-size:22px}header p{margin:3px}.pdf-house{break-before:page;page-break-before:always}.pdf-house:first-of-type{break-before:auto;page-break-before:auto}h2{padding:7px;margin-bottom:3px;background:#d9eee5;text-align:center;font-size:14px}.section-meta{text-align:center;margin:0 0 7px;color:#52645d}table{width:100%;border-collapse:collapse;page-break-inside:auto}thead{display:table-header-group}tr{page-break-inside:avoid}th,td{border:1px solid #71817b;padding:4px;text-align:center}th{background:#142b25;color:white}.left{text-align:left}.total td{background:#edf4f1;font-weight:bold}.summary{font-size:11px}.print-footer{position:fixed;bottom:-6mm;right:0;color:#60706a;font-size:8px}.page:after{content:counter(page)}</style></head><body><header><h1>AcadPulse</h1><p>Academic Intelligence Platform</p><strong>${escapeHtml(state.details.exam)} – Scoreboard</strong><p>Class ${escapeHtml(state.details.className)} – Section ${escapeHtml(state.details.section)}</p></header><footer class="print-footer">AcadPulse · Page <span class="page"></span></footer>${entries.map(([h,s])=>table(h,s)).join("")}${summary}<script>window.onload=()=>window.print()<\/script></body></html>`;
  }
