...
def current_user(access: AccessContext = Depends(get_current_access)) -> CurrentUserRead:
    return CurrentUserRead(
        ...
        assignments=[AccessAssignmentRead(
            role_code=a.role_code, role_name=a.role_name, scope_type=a.scope_type,
            organization_id=a.organization_id, institution_id=a.institution_id,
            academic_division_id=a.academic_division_id,
        ) for a in access.assignments],
    )


def _is_management_admin(access: AccessContext) -> bool:
    return any(a.role_code == "MANAGEMENT_ADMIN" and a.scope_type == "organization" and a.organization_id is not None for a in access.assignments)


def _validate_management_target(payload: AdminUserCreate, access: AccessContext, db: Session) -> tuple[Role, str, None, object]:
    if payload.role_code.strip().upper() != "PRINCIPAL":
        raise HTTPException(status_code=403, detail="Management / Group Admin may provision Principal accounts only")
    ...
    if institution.organization_id not in access.organization_ids:
        raise HTTPException(status_code=403, detail="Principal can only be assigned to an institution within your organization")
...
def _validate_platform_target(payload: AdminUserCreate, db: Session) -> tuple[Role, str, object, object]:
...
def create_or_reset_scoped_user(payload: AdminUserCreate, db: Session = Depends(get_db), access: AccessContext = Depends(get_current_access)) -> AdminUserRead:
    if access.is_platform_admin:
        ...
        raise HTTPException(status_code=403, detail="User provisioning requires Platform Admin or Management / Group Admin access")
    ...
    if user is not None and not access.is_platform_admin:
        active_assignments = db.execute(select(UserRoleAssignment, Role).join(Role, Role.id == UserRoleAssignment.role_id).where(UserRoleAssignment.user_id == user.id, UserRoleAssignment.is_active.is_(True), Role.is_active.is_(True))).all()
        for assignment, existing_role in active_assignments:
            if existing_role.code != "PRINCIPAL" or assignment.institution_id is None:
                raise HTTPException(status_code=403, detail="Management cannot reset an account assigned to another role")
            ...
            if existing_institution is None or existing_institution.organization_id not in access.organization_ids:
                raise HTTPException(status_code=403, detail="Management cannot reset a user outside its organization")
    ...
    if user is None:
        ...
        user.display_name = payload.display_name.strip(); user.status = "active"; user.is_platform_admin = False; user.password_hash = hash_password(payload.password)

    for assignment in db.scalars(select(UserRoleAssignment).where(UserRoleAssignment.user_id == user.id)).all():
        assignment.is_active = False
    assignment = db.scalar(select(UserRoleAssignment).where(UserRoleAssignment.user_id == user.id, UserRoleAssignment.role_id == role.id, UserRoleAssignment.scope_type == scope_type, UserRoleAssignment.organization_id == organization_id, UserRoleAssignment.institution_id == institution_id, UserRoleAssignment.academic_division_id.is_(None)))
    if assignment is None:
        db.add(UserRoleAssignment(user_id=user.id, role_id=role.id, scope_type=scope_type, organization_id=organization_id, institution_id=institution_id, academic_division_id=None, is_active=True))
    else:
        assignment.is_active = True
    db.commit(); db.refresh(user)
    return AdminUserRead(id=user.id, email=user.email, display_name=user.display_name, role_code=role.code, role_name=role.name, scope_type=scope_type, organization_id=organization_id, institution_id=institution_id)
